...
int main(int argc, char** argv) {
    sqlite3 *db;
    setupDB("test.sqlite");
    changeUser("test.sqlite","user","test");
...
int setupDB(char* dbname){
    ...
    if(sqlite3_open(dbname, &db) == SQLITE_OK) {
        ...
        const char *create_user_table = "CREATE TABLE IF NOT EXISTS user('username' VARCHAR, 'password' VARCHAR)";
        ...
        if (sqlite3_exec(db, create_user_table, NULL, NULL, &errMsg) == SQLITE_OK && sqlite3_exec(db, create_playlist_table, NULL, NULL, &errMsg) == SQLITE_OK) {
...
int changeUser(char* dbname, char* username, char* password){
    sqlite3 *db;

    char *errMsg;

    if(sqlite3_open(dbname, &db) == SQLITE_OK) {
        const char *delete ="DELETE FROM user";
        const char insert[256];
        snprintf(insert, sizeof insert, "%s%s%s%s%s","INSERT INTO user(username, password) VALUES('",username,"', '",password,"');");
        if (sqlite3_exec(db, delete, NULL, NULL, &errMsg) == SQLITE_OK && sqlite3_exec(db, insert, NULL, NULL, &errMsg) == SQLITE_OK) {
            printf("User added");
            sqlite3_close(db);
            return 1;
        } else {
